...
from loss import VaeLoss, VaeLossAuxiliary
...
def train(cfg):
    print("# Voice Quality Learning VAE training script")
    ...
    ds_train = VocalSetDataset(cfg.datapath_train, sr=cfg.sr, duration=cfg.duration, transform=preproc_train, augm_transform=preproc_train_augm)
    ds_valid = VocalSetDataset(cfg.datapath_valid, sr=cfg.sr, duration=cfg.duration, transform=preproc_valid)
    ...
    n_auxiliaryfeats = ds_train.auxiliary_features_lengths
    ...
    # what does chart_dependencies do?
    chart_dependencies(model, input_shape=(cfg.batch_size, n_channels, n_features, n_timeframes))
    print(model.get_infos())

    run_name = get_now() #run name == date and time
    wandb_run = wandb.init(
        name=run_name,
        project='vae-voicequality-multitask',
        entity='vocex',
        config=cfg.__dict__, #pass config file to WandB init as dict
        job_type='train'
    )
    wandb_run.watch(model, log='all') #why do we need to do this?
    ...
    ds_test = VocalSetDataset(cfg.datapath_test, sr=cfg.sr, duration=cfg.duration, pitches=[60], transform=preproc_valid, label='full')
...
def training_loop(epochs, model, optimizer, dl_train, dl_valid, val_every, kld_weight, kld_exp, device='cpu', wandb_run=None, log_pictures=False):
    ...
    for epoch in range(1, epochs + 1):
        ...
        # KLD weight
        # model.loss.kld_weight = kld_weight #for static KLD weight
        model.loss.kld_weight = kld_scheduler(kld_weight, kld_exp, epoch) #for varying KLD weight
        train_one_epoch(
            model, 
            dl_train, 
            optimizer, 
